...
export const EntryHistorySidebar: React.FC<EntryHistorySidebarProps> = ({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:sticky top-0 inset-y-0 left-0 z-40 w-80 bg-[#fbfbfa] border-r border-stone-200/80 flex flex-col h-screen shrink-0 transform transition-transform duration-200 ease-in-out ${
          isOpenMobile ? "translate-x-0" : "-translate-x-full md:translate-x-0"
        }`}
      >
        {/* Top Action Bar */}
        <div className="p-4 border-b border-stone-200/70 bg-white/70 backdrop-blur-xs">
          <button
            id="new-entry-btn"
            onClick={() => {
              onNewEntry();
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-stone-900 text-stone-50 hover:bg-stone-800 active:scale-[0.98] font-medium text-xs sm:text-sm transition-all shadow-xs cursor-pointer"
          >
        ...
        <div className="px-4 py-2 text-[11px] font-medium uppercase tracking-wider text-stone-500 flex items-center justify-between border-b border-stone-200/50">
        ...
        {/* Entries List */}
        <div className="flex-1 overflow-y-auto px-2 pb-4 space-y-1.5 pt-2">
              ...
              return (
                <div
                  ...
                  className={`group relative p-3 rounded-xl cursor-pointer transition-all border text-left ${
                    isActive
                      ? "bg-white border-stone-300 shadow-sm ring-1 ring-stone-900/5 before:absolute before:left-0 before:top-2.5 before:bottom-2.5 before:w-1 before:bg-stone-900 before:rounded-r"
                      : "bg-white/40 hover:bg-white border-transparent hover:border-stone-200/80 text-stone-700"
                  }`}
                >
                  ...
                  <p className="text-[11px] text-stone-500 line-clamp-2 mt-1 leading-relaxed pl-1">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {interactionCount > 0 && (
                        <span className="inline-flex items-center gap-0.5 text-stone-600">
                          <MessageSquare className="w-2.5 h-2.5 text-stone-400" />
                          {interactionCount}
                        </span>
                      )}
                      {entry.summary && (
            ...
            <div className="bg-white rounded-2xl max-w-sm w-full p-5 shadow-xl border border-stone-200">
              ...
              <div className="mt-4 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-medium bg-red-600 text-white hover:bg-red-700 rounded-lg shadow-xs cursor-pointer"
                >
                  Delete Entry
                </button>
              </div>
